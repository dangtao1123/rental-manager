...
function normalizeRentalCheckout(input, current = {}) {
  ...
  const propertyAmount = rentalMoney(input.propertyAmount ?? current.propertyAmount);
  const totalDeduction = Math.round((waterAmount + electricityAmount + propertyAmount + otherAmount) * 100) / 100;
