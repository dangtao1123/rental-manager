(() => {
  ...
  updateCheckoutAmounts = function () {
    ...
    const summary = $('#checkout-refund-summary'); if (summary) { summary.hidden = false; summary.innerHTML = `<div class="refund-summary-title">应退费用 <strong>${refund.toFixed(2)}元</strong></div><div class="refund-summary-grid"><span>押金：${formatAmount(deposit)}</span><span class="expense-text">水费扣除：${formatAmount(water)}</span><span>${electricityDetail}</span>${otherDetail}</div>`; }
  };
...
const rows = [...groups.entries()].map(([roomNo, items]) => [roomLabel(roomNo), `<div class="item-chip-list">${items.map((item) => `<span class="item-chip"><strong>${esc(item.name)} × ${esc(Number(item.quantity || 1))}</strong>${item.note ? `：${esc(item.note)}` : ''}${item.image ? ` <img class="item-thumb item-preview-trigger" src="${esc(item.image)}" data-preview-image="${esc(item.image)}" alt="${esc(item.name)}" />` : ''}</span>`).join('')}</div>`, items.reduce((sum, item) => sum + Math.max(1, Number(item.quantity || 1) || 1), 0), `<button class="small" data-show-room-items="${esc(roomNo)}">查看清单</button>`]);
  ...
  document.addEventListener('input', (event) => { if (event.target.closest('#checkout-other-items')) { syncOtherItems(); updateCheckoutAmounts(); } if (event.target.name === 'monthlyRent' || event.target.name === 'monthlyPropertyFee') { updateRenewalAmount(); updateCheckoutAmounts(); updateLeaseTotals(); } if (event.target.id === 'overview-finance-start' || event.target.id === 'overview-finance-end') { overviewFinanceFilter.mode = 'custom'; overviewFinanceFilter.start = document.getElementById('overview-finance-start')?.value || ''; overviewFinanceFilter.end = document.getElementById('overview-finance-end')?.value || ''; renderStats(); } });
