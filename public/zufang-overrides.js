(() => {
  ...
  function flowFieldMarkup(type, record, field) {
    ...
    if (inputType === 'readonly') { const labels = { monthly: '月付', quarterly: '季付', yearly: '年付' }; const displayValue = key === 'paymentMethod' ? (labels[value] || value) : value; const display = displayValue === 0 ? '0' : (displayValue || '未设置'); return `<div class="${className} readonly-field"><span class="field-label">${esc(label)}</span><strong data-readonly-value="${esc(key)}">${esc(display)}</strong><input type="hidden" name="${key}" value="${esc(value)}" /></div>`; }
  ...
  function mobileFlowSteps(type) {
    const labels = type === 'renewal' ? ['选择方案', '确认支付', '完成续费'] : ['退房登记', '费用结算', '完成退房'];
    return `<div class="mobile-flow-steps" aria-label="${labels.join('、')}">${labels.map((label, index) => `<div class="mobile-flow-step ${index === 0 ? 'active' : ''}"><b>${index + 1}</b><span>${label}</span>${index < labels.length - 1 ? '<i aria-hidden="true"></i>' : ''}</div>`).join('')}</div>`;
  }
  ...
  function mobileFlowMarkup(type, record) {
    if (type === 'renewal') {
      return `<div class="mobile-flow-sheet mobile-renewal-flow">
        ...
        ${mobileFlowSteps(type)}
        <section class="flow-card flow-summary-card"><div class="flow-card-head"><div><span class="flow-card-kicker">当前租约</span><h3>租户与房源</h3></div><span class="flow-state">在租</span></div><div class="flow-summary-grid renewal-summary-grid">${flowFieldSet(type, record, ['roomNo', 'tenantName', 'tenantPhone', 'leaseStart', 'endDate'])}</div></section>
        <section class="flow-card flow-choice-card"><div class="flow-card-head"><div><span class="flow-card-kicker">续租方案</span><h3>选择续租时长</h3></div><span class="flow-card-help">开始日期会自动衔接</span></div><div class="flow-grid-two">${flowFieldSet(type, record, ['renewalDate', 'durationPreset', 'durationValue', 'startDate', 'endDate'])}</div></section>
        <section class="flow-card flow-renewal-settings-card"><div class="flow-card-head"><div><span class="flow-card-kicker">租期与费用</span><h3>租金和物业费</h3></div><span class="flow-card-help">可按本次续租调整</span></div><div class="renewal-setting-grid"><div class="renewal-setting-card renewal-rent-card"><strong>租金设置</strong>${flowFieldSet(type, record, ['monthlyRent'])}</div><div class="renewal-setting-card renewal-property-card"><strong>物业费设置</strong>${flowFieldSet(type, record, ['monthlyPropertyFee'])}</div></div></section>
        <section class="flow-card flow-fee-card flow-renewal-fee-card"><div class="flow-card-head"><div><span class="flow-card-kicker">费用明细</span><h3>本次应收</h3></div><span class="flow-card-help">按所选周期计算</span></div><div class="flow-receipt renewal-receipt"><div class="flow-receipt-row"><span>月租金</span><strong data-flow-renewal-rent>0.00元</strong></div><div class="flow-receipt-row"><span>物业费</span><strong data-flow-renewal-fee>0.00元</strong></div><div class="flow-receipt-row"><span>押金</span><strong data-flow-renewal-deposit>0.00元</strong></div><div class="flow-receipt-row flow-receipt-period"><span>计费周期</span><strong data-flow-renewal-period>1个月</strong></div></div><div class="flow-total-row"><span>本次应收</span><strong class="flow-amount-value" data-flow-amount>0.00元</strong></div></section>
        <section class="flow-card flow-payment-card"><div class="flow-card-head"><div><span class="flow-card-kicker">支付方式</span><h3>沿用租约设置</h3></div><span class="flow-card-help">可在租户信息中修改</span></div>${flowFieldSet(type, record, ['paymentMethod'])}</section>
    ...
    return `<div class="mobile-flow-sheet mobile-checkout-flow">
      ...
      ${mobileFlowSteps(type)}
      <section class="flow-card flow-summary-card"><div class="flow-card-head"><div><span class="flow-card-kicker">当前租约</span><h3>租户与房源</h3></div><span class="flow-state">待退房</span></div><div class="flow-summary-grid">${flowFieldSet(type, record, ['roomDisplay', 'tenantName', 'tenantPhone', 'leaseStart', 'leaseEnd', 'paidThrough'])}</div></section>
      ...
      <section class="flow-card flow-meter-card"><div class="flow-card-head"><div><span class="flow-card-kicker">表数抄录</span><h3>水电表读数</h3></div><span class="flow-card-help">填写退房时读数</span></div><div class="flow-meter-table"><div class="flow-meter-table-head"><span>项目</span><span>入住</span><span>退房</span><span>用量</span><span>单价</span><span>扣款</span></div><div class="flow-meter-row"><strong class="flow-meter-name">水表</strong>${flowFieldSet(type, record, ['waterStart', 'waterEnd'])}<span class="flow-meter-computed" data-flow-water-usage>--</span>${flowFieldSet(type, record, ['waterUnitPrice', 'waterAmount'])}</div><div class="flow-meter-row"><strong class="flow-meter-name">电表</strong>${flowFieldSet(type, record, ['electricityStart', 'electricityEnd'])}<span class="flow-meter-computed" data-flow-electricity-usage>--</span>${flowFieldSet(type, record, ['electricityUnitPrice', 'electricityAmount'])}</div></div></section>
      <section class="flow-card flow-fee-card flow-settlement-card"><div class="flow-card-head"><div><span class="flow-card-kicker">费用结算</span><h3>扣费与押金</h3></div><span class="flow-card-help">核对后再确认</span></div><div class="flow-receipt checkout-receipt"><div class="flow-receipt-row"><span>押金应退</span><strong data-flow-deposit>0.00元</strong></div><div class="flow-receipt-row"><span data-flow-water-label>水费扣款</span><strong class="expense-text" data-flow-water-deduction>-0.00元</strong></div><div class="flow-receipt-row"><span data-flow-electricity-label>电费扣款</span><strong data-flow-electricity-deduction>-0.00元</strong></div><div class="flow-receipt-edit-row">${flowFieldSet(type, record, ['propertyAmount'])}</div></div><div class="flow-other-items-wrap"><div class="flow-subsection-head"><strong>其他扣除项</strong><span>可选</span></div>${flowFieldSet(type, record, ['otherItems'])}<div class="flow-receipt-row flow-other-total-row"><span>其他扣除合计</span><strong data-flow-other-total>-0.00元</strong></div></div><div class="flow-total-row"><span>预计应退</span><strong class="flow-refund-value" data-flow-refund>0.00元</strong></div><p class="flow-settlement-note">扣款为负数，表示从押金中扣除的费用。</p></section>
      <section class="flow-card flow-bank-card"><div class="flow-card-head"><div><span class="flow-card-kicker">退款账户</span><h3>收款信息</h3></div><span class="flow-card-help">用于退还押金</span></div><div class="flow-grid-two flow-bank-grid">${flowFieldSet(type, record, ['bankName', 'accountName', 'accountNo'])}</div></section>
      <section class="flow-card flow-note-card"><div class="flow-card-head"><div><span class="flow-card-kicker">补充信息</span><h3>备注</h3></div><span class="flow-card-help">选填</span></div>${flowFieldSet(type, record, ['note'])}</section>
    </div>`;
  }
  ...
  updateRenewalAmount = function () {
    ...
    const formatAmount = (value) => `${Number(value || 0).toFixed(2)}元`;
    const flowAmount = formatAmount(amount);
    document.querySelectorAll('[data-flow-amount]').forEach((node) => { node.textContent = flowAmount; });
    document.querySelectorAll('[data-flow-renewal-rent]').forEach((node) => { node.textContent = formatAmount(rent * (preset === 'days' ? days / 30 : (months[preset] || 1))); });
    document.querySelectorAll('[data-flow-renewal-fee]').forEach((node) => { node.textContent = formatAmount(fee * (preset === 'days' ? days / 30 : (months[preset] || 1))); });
    document.querySelectorAll('[data-flow-renewal-deposit]').forEach((node) => { node.textContent = formatAmount(deposit); });
    document.querySelectorAll('[data-flow-renewal-period]').forEach((node) => { node.textContent = preset === 'days' ? `${days}天` : `${months[preset] || 1}个月`; });
  ...
  updateCheckoutAmounts = function () {
    ...
    const water = Math.max(0, Math.round((endWater - startWater) * waterPrice * 10) / 10); const electricity = Math.round((startElectricity - endElectricity) * electricityPrice * 10) / 10; const propertyAmount = Number($('#dialog-fields [name="propertyAmount"]')?.value || 0); const otherRows = syncOtherItems(); const other = otherRows.reduce((sum, row) => sum + (row.mode === 'refund' ? -Number(row.amount || 0) : Number(row.amount || 0)), 0); const deposit = Number($('#dialog-fields [name="deposit"]')?.value || 0); const refund = Math.round((deposit - water - electricity - propertyAmount - other) * 10) / 10;
    ...
    const formatAmount = (value) => `${Math.abs(Number(value || 0)).toFixed(2)}元`;
    document.querySelectorAll('[data-flow-refund]').forEach((node) => { node.textContent = `${refund.toFixed(2)}元`; });
    document.querySelectorAll('[data-flow-deposit]').forEach((node) => { node.textContent = formatAmount(deposit); });
    document.querySelectorAll('[data-flow-water-deduction]').forEach((node) => { node.textContent = `-${formatAmount(water)}`; });
    document.querySelectorAll('[data-flow-electricity-label]').forEach((node) => { node.textContent = electricity < 0 ? '电费返还' : '电费扣款'; });
    document.querySelectorAll('[data-flow-electricity-deduction]').forEach((node) => { node.textContent = electricity < 0 ? `+${formatAmount(electricity)}` : `-${formatAmount(electricity)}`; node.classList.toggle('income-text', electricity < 0); node.classList.toggle('expense-text', electricity >= 0); });
    document.querySelectorAll('[data-flow-property-deduction]').forEach((node) => { node.textContent = `-${formatAmount(propertyAmount)}`; });
    document.querySelectorAll('[data-flow-other-total]').forEach((node) => { node.textContent = other < 0 ? `+${formatAmount(other)}` : `-${formatAmount(other)}`; node.classList.toggle('income-text', other < 0); node.classList.toggle('expense-text', other >= 0); });
    const waterUsage = Math.max(0, endWater - startWater); const electricityUsage = endElectricity - startElectricity;
    document.querySelectorAll('[data-flow-water-usage]').forEach((node) => { node.textContent = `${waterUsage.toFixed(1)} 吨`; });
    document.querySelectorAll('[data-flow-electricity-usage]').forEach((node) => { node.textContent = `${electricityUsage.toFixed(1)} 度`; });
    const electricityDetail = electricity < 0 ? `<span class="income-text">电费返还：${formatAmount(electricity)}</span>` : `<span class="expense-text">电费扣除：${formatAmount(electricity)}</span>`;
    const propertyDetail = `<span class="expense-text">物业/维修扣款：${formatAmount(propertyAmount)}</span>`;
    const otherDetail = otherRows.filter((row) => row.item || Number(row.amount)).map((row) => `<span class="${row.mode === 'refund' ? 'income-text' : 'expense-text'}">${row.mode === 'refund' ? '返还' : '扣除'}：${esc(row.item || '其他项目')} ${formatAmount(row.amount)}</span>`).join('');
    const summary = $('#checkout-refund-summary'); if (summary) { summary.hidden = false; summary.innerHTML = `<div class="refund-summary-title">应退费用 <strong>${refund.toFixed(2)}元</strong></div><div class="refund-summary-grid"><span>押金：${formatAmount(deposit)}</span><span class="expense-text">水费扣除：${formatAmount(water)}</span><span>${electricityDetail}</span><span>${propertyDetail}</span>${otherDetail}</div>`; }
  };
...
const rows = [...groups.entries()].map(([roomNo, items]) => [roomLabel(roomNo), `<div class="item-chip-list">${items.map((item) => `<span class="item-chip"><strong>${esc(item.name)} × ${esc(Number(item.quantity || 1))}</strong>${item.note ? `：${esc(item.note)}` : ''}${item.image ? ` <img class="item-thumb item-preview-trigger" src="${esc(item.image)}" data-preview-image="${esc(item.image)}" alt="${esc(item.name)}" />` : ''}</span>`).join('')}</div>`, items.reduce((sum, item) => sum + Math.max(1, Number(item.quantity || 1) || 1), 0), `<button class="small" data-show-room-items="${esc(roomNo)}">查看清单</button>`]);
  ...
  renderLedger = function () {
    ...
    const ledgerRows = list.map((item) => { const incomeRow = item.direction === 'income'; const sourceMaintenance = item.sourceType === 'maintenance' ? state.maintenance.find((maintenance) => maintenance.id === item.sourceId) : null; const note = sourceMaintenance ? [sourceMaintenance.item, sourceMaintenance.note].filter((part, index, parts) => part && (index === 0 || part !== parts[0])).join('·') : (item.note || ''); return [`<span class="${incomeRow ? 'income-text' : 'expense-text'}">${item.recordDate}</span>`, roomLabel(item.roomNo), `<span class="${incomeRow ? 'income-text' : 'expense-text'}">${incomeRow ? '收入' : '支出'}</span>`, `<span class="${incomeRow ? 'income-text' : 'expense-text'}">${money(item.amount)}</span>`, labels[item.category] || item.category, `<span class="ledger-note">${esc(note)}</span>`, `<div class="table-actions ledger-actions"><button class="small icon-button ledger-edit" data-edit="ledger" data-id="${item.id}" title="编辑" aria-label="编辑"><iconify-icon icon="hugeicons:edit-02" aria-hidden="true"></iconify-icon></button><button class="small danger icon-button" data-delete="ledger" data-id="${item.id}" title="归档" aria-label="归档"><iconify-icon icon="hugeicons:archive-02" aria-hidden="true"></iconify-icon></button></div>`]; });
    $('#ledger-list').innerHTML = table(['日期', '房间', '收支类型', '金额', '分类', '备注', '操作'], ledgerRows);
  };
